...
OUTFILE="classifier_catalog_results.json"
...
#    ;;
		...
		--catalogfile=*)
			CATALOGFILE=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
			if [ "$CATALOGFILE" != "" ]; then
				CATALOGFILE_GIVEN=true
			fi
		;;
		
		## CATALOG OPTIONS
		--catalog-level=*)
			CATALOG_LEVEL=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
		;;
		--coordinate-mode=*)
			COORDINATE_MODE=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
		;;
		--size-mode=*)
			SIZE_MODE=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
		;;
		--cutout-size=*)
			CUTOUT_SIZE=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
		;;
		--cutout-size-arcsec=*)
			CUTOUT_SIZE_ARCSEC=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
		;;
		--cutout-margin=*)
			CUTOUT_MARGIN=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
		;;
		--cutout-margin-factor=*)
			CUTOUT_MARGIN_FACTOR=`echo $item | /bin/sed 's/[-a-zA-Z0-9]*=//'`
		;;
		--save-cutouts*)
			SAVE_CUTOUTS="--save_cutouts"
		;;
		--overwrite-class-fields*)
			OVERWRITE_CLASS_FIELDS="--overwrite_class_fields"
		;;
...
if [ "$INPUTFILE_GIVEN" = false ]; then
  echo "ERROR: Missing or empty INPUTFILE arg!"
  exit 1
fi

if [ "$CATALOGFILE_GIVEN" = false ]; then
  echo "ERROR: Missing or empty CATALOGFILE arg!"
  exit 1
fi
...
CATALOG_OPTS="--catalogfile=$CATALOGFILE --catalog_level=$CATALOG_LEVEL --coordinate_mode=$COORDINATE_MODE --size_mode=$SIZE_MODE --cutout_size=$CUTOUT_SIZE --cutout_margin=$CUTOUT_MARGIN --cutout_margin_factor=$CUTOUT_MARGIN_FACTOR $SAVE_CUTOUTS $OVERWRITE_CLASS_FIELDS "

if [ "$CUTOUT_SIZE_ARCSEC" != "" ]; then
